...
const { Pool } = pg;
const LOGO_BYTES = readFileSync(new URL("../C&A Full Logo.png", import.meta.url));
...
function translate(sql, params) {
  ...
  if (/^INSERT INTO/i.test(text) && !/ON CONFLICT/i.test(text)) {
    if (/task_statuses/i.test(text) || /task_tags/i.test(text) || /clients/i.test(text)) text += " ON CONFLICT DO NOTHING";
  }
  ...
  text = text
    ...
    .replace(/GROUP_CONCAT\(COALESCE\(parent\.name \|\| ' \| ', ''\) \|\| child\.name, ', '\)/g, "STRING_AGG(COALESCE(parent.name || ' | ', '') || child.name, ', ')");
...
export default async function handler(req, res) {
  const requestPath = decodeURIComponent(new URL(req.url || "/", "https://local.invalid").pathname);
  if (requestPath === "/C&A Full Logo.png") {
    res.statusCode = 200;
